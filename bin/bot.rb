  ...
  bot.listen do |message|
    ...
    when '/start'
      ...
      bot.api.send_message(chat_id: message.chat.id,
                           text: "I have to tell you #{message.from.first_name}, that I'm not an ordinary bot. I'm a 5 star bot.. Ask me 'why?'.")
      bot.api.send_message(chat_id: message.chat.id, text: "FYI, that means type 'why?'")
      bot.api.send_message(chat_id: message.chat.id,
                           text: "My bad.. FYI means 'for your information'.. And just touch to the buttons that has w, h and y letters on it, one by one. Forget the question mark, I don't want to confuse you.")

    when 'why'
      bot.api.send_message(chat_id: message.chat.id,
                           text: "Depends.. Also congrats, #{message.from.first_name}!! You did it. Now tell me, what do you want to say to me?")
    ...
    when 'Give me the best of all time'
      bot.api.send_message(chat_id: message.chat.id, text: recommedation.certainbest,
                           reply_markup: keyboard.keyboard(keyboard.second_base))
    ...
    when '/stop'
      ...
      bot.api.send_message(chat_id: message.chat.id,
                           text: "Just '/start' || '/stop' . It's not a rocket science #{message.from.first_name}.")
    end
  end
end
